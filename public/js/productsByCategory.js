...
const getAllProducts = async () => {
    ...
    const url = `http://localhost:3001/api/v1/products/${catId}`
    await fetch(url, {
            method: 'GET',
            headers: {
        ...
        .then((data) => {
            ...
            let catDesc = localStorage.getItem('categoryDescr')
            const productsDiv = document.querySelector('.categoryDesc')
            const catDescDiv = document.createElement('div')
            catDescDiv.innerHTML = catDesc
            catDescDiv.classList.add('container', 'fs-4')
            productsDiv.appendChild(catDescDiv)
        })
}
